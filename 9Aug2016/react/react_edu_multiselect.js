	        ...
	        var options = self.props.options.map(function (option) {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement('input', { type: 'checkbox', id: checkBoxBindId, value: option[self.props.valueField], onChange: this.handleChange , name: 'roles_checkbox' }),
	                    React.createElement(
	                        'label',
	        ...
	        var options = self.props.options.map(function (option) {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    'div',
	                    { className: 'checkbox custom' },
	                    React.createElement('input', { type: 'checkbox', id: option[self.props.idField], value: option[self.props.valueField], onChange: this.handleChange , name: 'fun_checkbox' }),
	                    React.createElement(
	                        'label',
	...
	var EducationList = React.createClass({
	    ...
	    getInitialState: function getInitialState() {
	        ...
	        return {
	            ...
	            search_box_value: [],
	            educatuionIdsArray: [],
	            displayeducationFlag : false,
	            displayEducation : 'none',
	        };
	    },
       ...
       getInitialState: function getInitialState() {
	        ...
	        var options = self.props.options.map(function (option) {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    { className: 'checkbox custom' },
	                    React.createElement('input', { type: 'checkbox', id: option[self.props.idField], value: option[self.props.valueField], 
	                    								onChange: this.handleChange, name: 'edu_checkbox' }),
	                    React.createElement(
	                        'label',
	                        { htmlFor: option[self.props.idField] },
	                        option[self.props.labelField]
	    ...
	    handleChange: function handleChange(e) {
	        if (e.target.checked) {
	            this.state.search_box_value.push(e.target.value);
	            this.state.educationIdsArray.push(e.target.id);
	        } else {
	            var index = this.state.search_box_value.indexOf(e.target.value);
                    var indexOfId = this.state.functionsIdsArray.indexOf(e.target.id);
	            if (index >= 0) {
	        ...
	        var urlToHitForStream = "http://my.monsterindia.noida/reactCombo.html?type=stream&ed="+ this.state.educationIdsArray.join();
            var functionData = $.getJSON(urlToHitForStream, function (data) {
		var optionsArr = [];
				console.log(data);
				for (var i = 0; i < data.length; i++) {
				    var currentObject = data[i]
			...
			console.log(optionsArr);
                ReactDOM.render(React.createElement(StreamList, { id: 'myDropdown',
                    options: optionsArr,
                    value: 'Stream1',
                    labelField: 'NAME',
                    valueField: 'NAME',
                    idField: 'ID'
                }), document.getElementById('target-spec'));
            }).done(function () {
                console.log("function second success");
            }).fail(function () {
                console.log("function error");
	        ...
	        var options = self.props.options.map(function (option) {
	            ...
	            return React.createElement(
	                ...
	                React.createElement(
	                    'div',
	                    { className: 'checkbox custom' },
	                    React.createElement('input', { type: 'checkbox', id: option[self.props.idField], value: option[self.props.valueField], onChange: this.handleChange , name: 'spec__checkbox' }),
	                    React.createElement(
	                        'label',
	                        { htmlFor: option[self.props.idField] },
	                        option[self.props.labelField]
...
var LocationList = React.createClass({
	    ...
	    render: function render() {
	    	console.log(this.state.displaylocationFlag);
	        var self = this;
	        var count = 1;
	        // var checkBoxBindId = ''; //Old way for giving IDs, replace by self.props.idField
	        var options = self.props.options.map(function (option) {
	        ...
	        return React.createElement(
	            ...
	            React.createElement(
	                'button',
	                { type: 'button', className: 'btn btn-block select_main', 'data-id': 'location', id : 'location-btn', 
	                	onClick : this.handleClick, onBlur : this.handleBlur },
	                React.createElement(
	                    'span',
	                    { className: 'text' },
	            ...
	            React.createElement(
	                'div',
	                { className: 'select_menu', id: 'location', style : {display : this.state.displaylocation}},
	                React.createElement(
	                    'div',
	                    { className: 'search-box' },
	                    React.createElement('input', { type: 'text', placeholder: 'Search in the list', className: 'form-control live-search', value: this.state.search_box_value })
	    ...
	    handleBlur: function(){
	    	console.log('blur called');
	    	this.setState({displaylocationFlag : false});
	    },

	    handleClick: function(){
		if(this.state.displaylocationFlag == true)
	    	{
	    		this.setState({displaylocation : 'none', displaylocationFlag : false});
	    		this.setState({className : 'rmv'});
	    	}
	    	else
	    	{
	    		this.setState({displaylocation : 'block', displaylocationFlag : true});
	    		this.setState({className : 'snv'});
	    	}
	    }
	});
	...
	$( window ).load(function() {
	  ...
	  $.ajax({
	        ...
	        success: function success(data) {
	            var options = data;
	            var dropDownOnChange = function dropDownOnChange(change) {};
	            ReactDOM.render(React.createElement(EducationList, { id: 'myDropdown',
	                options: options,
	                value: 'Any',
	                labelField: 'NAME',
	                valueField: 'NAME',
	                idField: 'ID',
	                onChange: dropDownOnChange
	            }), document.getElementById('target-education'));
	        },
	        error: function error(xhr, status, err) {
	            alert('error in Education ajax request');
	        }
	    });
...
$.ajax({
	...
	success: function success(data) {
	    var options = data;
	    var dropDownOnChange = function dropDownOnChange(change) {};
	    ReactDOM.render(React.createElement(LocationList, { id: 'myDropdown',
		options: options,
		value: 'Any',
		labelField: 'NAME',
		valueField: 'NAME',
		idField: 'ID',
		onChange: dropDownOnChange
	    }), document.getElementById('target-loc'));
	},
	error: function error(xhr, status, err) {
	    alert('error in Location ajax request');
	}
    });
